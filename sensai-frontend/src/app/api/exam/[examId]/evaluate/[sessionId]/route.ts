...
export async function POST(
  ...
  try {
    ...
    console.log(
      `[FRONTEND-API] POST /api/exam/${examId}/evaluate/${sessionId}`
    );
    ...
    if (!response.ok) {
      ...
      console.error(
        "[FRONTEND-API] Backend evaluation error:",
        response.status,
        errorText
      );
      return NextResponse.json(
...
export async function GET(
  ...
  try {
    ...
    const response = await fetch(
      `${backendUrl}/api/exam/${examId}/evaluation/${sessionId}`,
      {
        method: "GET",
        headers: {
          "x-user-id": userId
    ...
    if (!response.ok) {
      ...
      const errorText = await response.text();
      console.error(
        "Backend evaluation fetch error:",
        response.status,
        errorText
      );
      return NextResponse.json(
