...
export async function GET(
  ...
  try {
    ...
    console.log(
      `[FRONTEND-API] GET /api/exam/${examId}/evaluation/${sessionId}`
    );
    ...
    console.log(
      `[FRONTEND-API] Backend GET response status: ${response.status}`
    );
    ...
    if (!response.ok) {
      ...
      console.error(
        "[FRONTEND-API] Backend evaluation fetch error:",
        response.status,
        errorText
      );
      return NextResponse.json(
